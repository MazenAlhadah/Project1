...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    await purchase.update({ status: 'confirmed', confirmed_by: auth.user!.id, confirmed_at: new Date() });
    return NextResponse.json({ success: true, message: 'تم تأكيد الدفع بنجاح' });
  } catch (error) {
