...
export async function GET(req: NextRequest) {
  ...
  try {
    const admins = await User.findAll({ where: { role: 'admin' }, attributes: { exclude: ['password_hash'] } });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: 'تمت الإضافة', data: admin }, { status: 201 });
  } catch(e) { return NextResponse.json({ success: false }, { status: 500 }); }
}
