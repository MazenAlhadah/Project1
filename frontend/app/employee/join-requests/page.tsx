...
export default function JoinRequestsPage() {
  ...
  const handleReject = async (id: string) => {
    ...
    setProcessing(id);
    try { await usersAPI.reject(id); toast.success('تم رفض الطالب'); setRequests(p => p.filter(r => r.id !== id)); }
    catch (e: any) { toast.error(e?.response?.data?.message || 'حدث خطأ'); }
    finally { setProcessing(null); }
  };
  ...
  return (
    <DashboardLayout requiredRole={['employee','admin']} navItems={EMP_NAV}>
      ...
      <div className="page-header">
        <h1 className="page-title">طلبات الانضمام</h1>
        <p className="page-subtitle">مراجعة بيانات الطلاب الجدد وقبول أو رفض طلباتهم</p>
      </div>

      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 32 }}>📋</span>
          <div><p style={{ fontSize: 24, fontWeight: 800, color: 'var(--warning)' }}>{requests.length}</p><p style={{ fontSize: 13, color: 'var(--text-400)' }}>طلب انتظار</p></div>
        </div>
      </div>

      {loading ? <div style={{ textAlign: 'center', padding: 60 }}><div className="spinner" style={{ margin: 'auto' }} /></div>
      : requests.length === 0 ? (
        <div className="empty-state"><div className="empty-state-icon">🎉</div><h3>لا توجد طلبات معلقة</h3><p>كل الطلبات تمت مراجعتها</p></div>
      ) : (
            ...
            <tbody>
              {requests.map((req, i) => (
                <tr key={req.id}>
                  ...
                  <td>
                    {req.id_card_image_url ? (
                      <button className="btn btn-secondary btn-sm" onClick={() => setViewImage(req.id_card_image_url)}>👁️ عرض</button>
                    ) : <span style={{ color: 'var(--text-400)', fontSize: 13 }}>لا يوجد</span>}
                  </td>
                  <td style={{ color: 'var(--text-400)', fontSize: 13 }}>{new Date(req.created_at).toLocaleDateString('ar-EG')}</td>
                  <td>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button className="btn btn-success btn-sm" onClick={() => handleApprove(req.id)} disabled={processing === req.id}>
                        {processing === req.id ? <span className="spinner" style={{width:14,height:14}}/> : '✅ قبول'}
                      </button>
