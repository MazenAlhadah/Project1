...
export default function PaymentConfirmationsPage() {
  ...
  const load = () => purchasesAPI.getPending().then(r => setPurchases(r.data.data || [])).catch(() => toast.error('خطأ في التحميل')).finally(() => setLoading(false));
  useEffect(() => { load(); }, []);
  ...
  const handleReject = async (id: string) => {
    ...
    setProcessing(id);
    try { await purchasesAPI.reject(id); toast.success('تم رفض الطلب'); setPurchases(p => p.filter(r => r.id !== id)); }
    catch (e: any) { toast.error(e?.response?.data?.message || 'حدث خطأ'); }
    finally { setProcessing(null); }
  };
  ...
  return (
    <DashboardLayout requiredRole={['employee','admin']} navItems={EMP_NAV}>
      <div className="page-header">
        <h1 className="page-title">تأكيد المدفوعات</h1>
        <p className="page-subtitle">مراجعة طلبات شراء الكتب وتأكيد الدفع</p>
      </div>

      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 32 }}>💳</span>
          <div><p style={{ fontSize: 24, fontWeight: 800, color: 'var(--warning)' }}>{purchases.length}</p><p style={{ fontSize: 13, color: 'var(--text-400)' }}>طلب معلق</p></div>
        </div>
      </div>

      {loading ? <div style={{ textAlign: 'center', padding: 60 }}><div className="spinner" style={{ margin: 'auto' }} /></div>
      : purchases.length === 0 ? (
        <div className="empty-state"><div className="empty-state-icon">✅</div><h3>لا توجد طلبات معلقة</h3></div>
      ) : (
                ...
                <tr key={p.id}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button className="btn btn-success btn-sm" onClick={() => handleConfirm(p.id)} disabled={processing === p.id}>
                        {processing === p.id ? <span className="spinner" style={{width:14,height:14}}/> : '✅ تأكيد الدفع'}
                      </button>
